...
SITE = {
    ...
    "twitter": "",
    # Contact for /.well-known/security.txt (RFC 9116). A mailto: or https: URI.
    "security_contact": "mailto:isakruas@gmail.com",
    # Search-engine ownership verification tokens. Paste the value the console
    # gives you; empty entries are omitted. Keys map to provider meta names.
    "verification": {
...
# Stamped once per build() call (see below); used for security.txt expiry and
...
# Standard poster shown on every uploaded video before it plays — keeps the
# feed uniform and on-brand. Generated by build_og_assets().
VIDEO_POSTER = "/video-poster.png"


def _enhance_videos(html: str) -> str:
    """Give every <video> the standard brand poster and sane mobile defaults,
    unless the tag already sets its own. Authors just write
    `<video controls src=...>` and get a uniform thumbnail for free."""

    def repl(match: re.Match) -> str:
        attrs = match.group(1)
        if "poster=" not in attrs:
            attrs += f' poster="{VIDEO_POSTER}"'
        if "playsinline" not in attrs:
            attrs += " playsinline"
        return f"<video{attrs}>"

    return _VIDEO_RE.sub(repl, html)
...
def render_markdown(body: str) -> str:
    ...
    return _enhance_videos(_enhance_images(md.convert(body)))
...
def build(include_drafts: bool = False) -> None:
    ...
    # Sitemap, robots, highlight stylesheet, manifest, security.txt.
    ...
    write(OUTPUT / ".well-known" / "security.txt",
          build_security_txt(NOW + dt.timedelta(days=365)))
    write(OUTPUT / "sw.js", build_service_worker(BUILD_VERSION))
...
def build_security_txt(expires: dt.datetime) -> str:
    """RFC 9116 security.txt. Expires is required by the spec; we set it a year
    out from the build so it never serves a stale, past-dated policy."""
    contact = SITE["security_contact"]
    lines = [
        f"Contact: {contact}",
        f"Expires: {expires.strftime('%Y-%m-%dT%H:%M:%SZ')}",
        "Preferred-Languages: en, pt, es",
        f"Canonical: {SITE['url']}/.well-known/security.txt",
    ]
    return "\n".join(lines) + "\n"


# Service worker: an offline fallback only. It never caches HTML (so content is
# always fresh from the network); it serves a cached shell of static assets when
# offline. Bumping CACHE invalidates the old cache on the next visit.
SERVICE_WORKER = """\
/* generated by build.py — offline asset cache, network-first for everything. */
const CACHE = "isaklab-v{version}";
const ASSETS = [
  "/style.css", "/highlight.css", "/theme.js", "/search.js",
...
def build_service_worker(version: str) -> str:
    return SERVICE_WORKER.format(version=version)
...
def build_og_assets(output: Path) -> None:
    ...
    # --- Standard video poster (1:1) ------------------------------------ #
    # Shown on every uploaded <video> before play — uniform, on-brand thumb.
    # Square, matching the 1:1 videos recorded for the site.
    S = 1080
    f_poster = _load_font(mono_bold, 104)
    f_pfoot = _load_font(mono_bold, 32)
    f_pbadge = _load_font(mono_bold, 80)
    img = Image.new("RGB", (S, S), BG)
    d = ImageDraw.Draw(img)
    cx, cy = S // 2, 470
    # "ik" badge, top-left, matching favicon.svg / OG cards.
    d.rounded_rectangle((84, 84, 220, 220), radius=30, fill=ACCENT)
    bx = d.textbbox((0, 0), "ik", font=f_pbadge)
    d.text((152 - (bx[2] - bx[0]) / 2, 152 - (bx[3] - bx[1]) / 2 - bx[1]),
           "ik", font=f_pbadge, fill="#ffffff")
    # Play button: accent disc with a soft outer ring and a white triangle.
    r = 100
    d.ellipse((cx - r - 16, cy - r - 16, cx + r + 16, cy + r + 16), outline=ACCENT, width=4)
    d.ellipse((cx - r, cy - r, cx + r, cy + r), fill=ACCENT)
    d.polygon([(cx - 32, cy - 50), (cx - 32, cy + 50), (cx + 56, cy)], fill="#ffffff")
    # Wordmark, centered under the play button.
    wb = d.textbbox((0, 0), SITE["title"], font=f_poster)
    d.text((cx - (wb[2] - wb[0]) / 2, 690), SITE["title"], font=f_poster, fill=FG)
    # Domain footer.
    dom = SITE["url"].split("://")[-1]
    fb = d.textbbox((0, 0), dom, font=f_pfoot)
    d.text((cx - (fb[2] - fb[0]) / 2, 850), dom, font=f_pfoot, fill=FAINT)
    img.save(output / "video-poster.png", optimize=True)
